...
class HtmlGenerate 
{
    public function generateILVProduct($plaque, $category)
    {
        ...
        if ($plaque['active_plate'] == 0) {
            ...
            /* end */
            $html .= '<input class="form-control" name="link_plate" type="text" id="'.($plaque['category_id'] == 1 ? 'nomEtablissement' : 'link-input' ).'" placeholder="' . ($plaque['category_id'] == 5 ? 'Entrez votre Email' : 'Exemple: Pizza time paris') . '" required>';
            ...
            $html .= '<button class="btn bg-faded-danger btn-icon me-2" type="submit" data-bs-toggle="tooltip" title="Réinitialiser la plaque" name="reset-plate" onclick="return confirm(\'Êtes-vous sûr de réinitialiser la plaque ?\');" ><i class="ci-compare text-danger"></i></button>';
            $html .='</form>';
            /* end button reset *******************************************************/
            $html .= '</div>';
            ...
            $html .= '<input class="form-control" name="update_link" type="text"  id="'.($plaque['category_id'] == 1 ? $plaque['id'].'nomEtablissement' : 'link-input' ).'" placeholder="' . ($plaque['category_id'] == 5 ? 'Entrez votre nouvel Email' : 'Exemple: Pizza time paris') . '" required>';
                /* liste for autocomplite name etablissment  */
                if($plaque['category_id'] == 1){
