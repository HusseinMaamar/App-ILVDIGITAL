...
function getCateygoryPlate($plateId)
{
    $query = "SELECT category_id FROM plaques WHERE id = :id";
    $statement = $this->db->getDb('./config/config.ini')->prepare($query);
    $statement->bindParam(":id", $plateId, PDO::PARAM_INT);
    $statement->execute();
    $result = $statement->fetch(PDO::FETCH_ASSOC);
    $statement->closeCursor();
    return $result; // Fermez le statement
...
/* select tokenn plate for bulding link base  */
function getTokenPlateAlreadyActive($plateId)
{
    $query = "SELECT activation_token FROM plaques WHERE id = :id";
    $statement = $this->db->getDb('./config/config.ini')->prepare($query);
    $statement->bindParam(":id", $plateId, PDO::PARAM_INT);
    $statement->execute();
    $result = $statement->fetch(PDO::FETCH_ASSOC);
...
/* reset plaqte */
function restPlate($plateId, $base_link , $active_plate, $user_id)
{
    $query = "UPDATE plaques SET url_qr_code = :base_link , active_plate = :active_plate , user_id = :user_id WHERE id = :id";
    $statement = $this->db->getDb('./config/config.ini')->prepare($query);
    $statement->bindParam(':base_link', $base_link);
    $statement->bindParam(':active_plate', $active_plate);
    $statement->bindParam(':user_id', $user_id);
    $statement->bindParam(':id', $plateId);
    $q = $statement->execute();
    $statement->closeCursor();
    if ($q) {
        return true;
