...
function obtenirLienRedirection($numero_unique, $activation_token) {
    global $conn;

    $stmt = $conn->prepare("SELECT lien_avis_google FROM plaques WHERE numero_unique = ? AND activation_token = ?");
    $stmt->bind_param("s", $numero_unique);
    $stmt->bind_param("t", $activation_token);
    $stmt->execute();
...
$numero_unique = isset($_GET["plaque"]) ? $_GET["plaque"] : "";
...
<?php  if(!empty($lien_redirection)) { ?>
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <input type="text" name="numero_unique" required><br>
